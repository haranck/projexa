...
export const getAllIssues = async (filter: GetAllIssuesFilterProps) => {
  const response = await AxiosInstance.get(
    API_ROUTES.ISSUE.GET_ALL_ISSUES.replace(":projectId", filter.projectId),
    { params: filter },
  );
  return response.data;
};
...
export const updateEpic = async (data: UpdateEpicProps) => {
  const response = await AxiosInstance.patch(
    API_ROUTES.ISSUE.UPDATE_ISSUE.replace(":issueId", data.epicId),
    data,
  );
  return response.data;
};
...
export const deleteIssue = async (issueId: string) => {
  const response = await AxiosInstance.delete(
    API_ROUTES.ISSUE.DELETE_ISSUE.replace(":issueId", issueId),
  );
  return response.data;
};
...
export const getAttachmentUploadUrl = async (contentType: string) => {
  const response = await AxiosInstance.post(
    API_ROUTES.ISSUE.GET_ATTACHMENT_UPLOAD_URL,
    { contentType },
  );
  return response.data;
};
