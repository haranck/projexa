...
export const DraggableIssueItem = ({
    ...
    const statusColor = () => {
        switch (issue.status?.toUpperCase()) {
            ...
            case "HOLD":        return "bg-orange-500/10 border-orange-500/20 text-orange-400";
    ...
    const assignee = projectMembers.find(m => m.userId === issue.assigneeId);
    ...
    return (
        <div
            ...
            className={`
                ...
                ${isDragging ? 'bg-zinc-800/50 border-white/5 opacity-30 shadow-2xl' : 'hover:border-white/5'}
                ${isOverlay ? 'bg-[#1a1c22] border-blue-500/50 shadow-2xl scale-[1.02] opacity-100 cursor-grabbing' : 'hover:translate-x-1'}
            `}
        >
            {/* Issue info row */}
            <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider">{issue.issueType}</span>
                </div>
            </div>

            {/* Actions row */}
            <div className="flex items-center gap-2 sm:gap-4 shrink-0 pl-6 sm:pl-0" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="relative">
                    <button
                        ...
                        className={`px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[10px] font-bold border transition-all flex items-center gap-1.5 ${statusColor()}`}
                    >
                        <span className="hidden sm:inline">{issue.status?.toUpperCase().replace("_", " ") || "TODO"}</span>
                        <span className="sm:hidden">{issue.status?.toUpperCase().replace("_", " ").slice(0, 4) || "TODO"}</span>
                    ...
                    {activeStatusDropdownId === issue._id && (
                        <div className="absolute top-full right-0 mt-2 z-50 bg-[#1a1c22] border border-white/10 rounded-xl shadow-2xl py-1.5 min-w-[130px] animate-in fade-in slide-in-from-top-1">
                ...
                {/* Issue key (hidden on xs) */}
                <span className="hidden sm:inline-flex text-[10px] font-bold text-zinc-500 bg-white/5 px-2.5 py-1 rounded-lg border border-white/5 min-w-[60px] text-center">
                    {issue.key}
                </span>

                {/* Assignee avatar */}
                <div className="w-7 h-7 rounded-full bg-zinc-800 flex items-center justify-center overflow-hidden border border-white/10 ring-2 ring-transparent group-hover:ring-white/5 transition-all shrink-0">
                    {assignee?.user?.profilePicture ? (
                        <img src={assignee.user.profilePicture} alt="" className="w-full h-full object-cover" />
                    ) : (
                        <div className="w-full h-full bg-zinc-900 flex items-center justify-center text-[10px] font-bold text-zinc-600 capitalize">
                            {assignee?.user?.userName?.charAt(0) || <UserIcon className="w-3.5 h-3.5" />}
                        </div>
                    )}
                </div>
            </div>
