...
export const EpicSidebar = ({
    ...
    return (
        /*
         * On mobile: fixed overlay (full-screen sheet from the left)
         * On md+:     static inline sidebar that shrinks flex row
         */
        <div
            className={`
            ...
            `}
        >
            <div className="bg-[#14171f] border-r border-white/10 md:border md:rounded-[2rem] overflow-hidden flex flex-col h-full md:h-[calc(100vh-14rem)] md:sticky md:top-6 shadow-2xl">

                {/* Header */}
                <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between">
                    <div className="relative">
                        <h3 className="font-bold text-sm tracking-tight text-white mb-1 px-1">Epics</h3>
                        <div className="absolute -bottom-4 left-1 w-8 h-[2px] bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)] rounded-full" />
                    </div>
                    <button
                        onClick={onClose}
                                        ...
                                        <div className="flex items-center justify-between px-4 py-3.5">
                                            <div className="flex flex-col gap-1 min-w-0 flex-1">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-semibold text-zinc-600 bg-white/4 px-1.5 py-0.5 rounded">{epic.key}</span>
                                                </div>
                                        ...
                                        {isExpanded && (
                                            <div className="animate-in fade-in slide-in-from-top-1 duration-200">
                                                <div className="px-5 space-y-2.5 pb-4">
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <span className="text-[11px] font-semibold text-zinc-300">
                                                            {epic.startDate ? new Date(epic.startDate as string).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Not set'}
                                                        </span>
                                                    </div>
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[11px] text-zinc-600">End Date</span>
                                                        <span className="text-[11px] font-semibold text-zinc-300">
                                                            {epic.endDate ? new Date(epic.endDate as string).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Not set'}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="px-5 pb-4">
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); onIssueClick(epic._id); }}
                                                    ...
                                                    <div className="px-3 pb-3 space-y-0.5">
                                                        {epicTasks.length === 0 ? (
                                                            ...
                                                            epicTasks.map((task, ti) => (
                                                                <div
                                                                    key={task._id}
                                                                    onClick={(e) => { e.stopPropagation(); onIssueClick(task._id); }}
