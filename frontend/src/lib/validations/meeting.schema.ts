...
export const scheduleMeetingSchema = baseMeetingSchema.refine((data) => {
...
// Reschedule schema — does NOT enforce startTime > now
// (existing meetings may keep their times or be moved slightly)
export const rescheduleMeetingSchema = baseMeetingSchema.refine((data) => {
    if (!data.startTime || !data.endTime) return true;
    const startDateTime = new Date(`${data.date}T${data.startTime}`);
    const endDateTime = new Date(`${data.date}T${data.endTime}`);
    return endDateTime > startDateTime;
