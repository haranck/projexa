...
const DashboardNavbar = () => {
    ...
    const { data: projectsData, isLoading: isProjectsLoading } = useGetAllProjects({ workspaceId: currentWorkspace?._id || currentWorkspace?.id || '' });
    ...
    useEffect(() => {
        ...
        if (workspaceProjects) {
            ...
            const workspaceId = currentWorkspace?._id || currentWorkspace?.id || '';
            const isProjectFromOtherWorkspace = currentProject &&
            ...
            <nav className="fixed top-0 left-64 right-0 z-30 h-20 bg-[#0b0e14]/80 backdrop-blur-md">
                <div className="flex items-center justify-between h-full px-8">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex flex-col gap-0.5 mt-1 relative" ref={projectDropdownRef}>
                            <div className="flex items-center gap-1 text-[9px] font-bold text-zinc-500 uppercase tracking-widest pl-0.5">
                                <div className="w-2.5 h-[1.5px] bg-zinc-700 rounded-full" />
                                Projects
                            </div>
                            ...
                            >
                                ...
                                <span className="text-xs font-semibold text-zinc-400 group-hover:text-white transition-colors">
                                    {currentProject?.projectName || (isProjectsLoading ? "Loading..." : "Select Project")}
                                </span>
                                <ChevronDown className={`h-3.5 w-3.5 text-zinc-500 group-hover:text-zinc-300 transition-transform ${isProjectDropdownOpen ? 'rotate-180' : ''}`} />
                            </button>
                                        ...
                                        {projects?.length === 0 ? (
                                            ...
                                            projects?.map((p: Project) => (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm transition-all ${currentProject?._id === p._id
                                                        ? "bg-emerald-500/10 text-emerald-400"
                                                        : "text-zinc-400 hover:text-white hover:bg-white/5"
                                                        }`}
                                                >
                                                    <div className={`w-1.5 h-1.5 rounded-full ${currentProject?._id === p._id ? 'bg-emerald-500' : 'bg-zinc-700'}`} />
                                                    <span className="flex-1 text-left truncate font-medium">{p.projectName}</span>
                                                    {currentProject?._id === p._id && <Check className="h-3.5 w-3.5" />}
                                                </button>
                                            ))
                                        )}
                                    </div>
