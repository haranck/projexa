...
export class UserController {
  constructor(
    @inject("IVerifyPasswordUseCase")
    private readonly _verifyPasswordUseCase: IVerifyPasswordUseCase,
    @inject("IProfileImageUploadUrlUseCase")
    private readonly _profileImageUploadUrlUseCase: IProfileImageUploadUrlUseCase,
  ...
  verifyPassword = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    res
      ...
      .json({ message: MESSAGES.USERS.PASSWORD_VERIFIED_SUCCESSFULLY });
  };
  getProfileImageUploadUrl = async (
    ...
    const result = await this._profileImageUploadUrlUseCase.execute({
      userId,
      contentType,
    });
    res
      .status(HTTP_STATUS.OK)
      .json({
        message: MESSAGES.USERS.PROFILE_IMAGE_UPLOAD_URL_GENERATED_SUCCESSFULLY,
        data: result,
      });
  };
  updateProfileImage = async (
    ...
    const result = await this._updateProfileImageUseCase.execute({
      userId,
      profileImage,
    });
